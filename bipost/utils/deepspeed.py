...
class DeepspeedStrategy(ABC):
    ...
    def create_optimizer(self, model, **kwargs) -> Optimizer:
        if isinstance(model, Actor):
            model = model.model
        # Optimizer - Use standard Adam to avoid CUDA compilation issues
        from torch.optim import Adam
        optim_params = get_optimizer_grouped_parameters(model, kwargs["weight_decay"])
        optim = Adam(optim_params, **kwargs)
        return optim
    ...
    def get_ds_eval_config(self, offload=False):
        # DS Config
        # Use ZeRO-0 for eval models (faster, no parameter gathering overhead)
        # ZeRO-2/3 require optimizers or have communication overhead that slows down eval
        # CPU offloading provides memory savings without the ZeRO overhead
        ds_config = get_eval_ds_config(offload=offload, stage=0, bf16=self.bf16)
        ds_config["train_micro_batch_size_per_gpu"] = self.micro_train_batch_size
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.is_rank_0():
            ...
            if isinstance(model_to_save, PeftModel):
                print(f"LoRA model detected, skipping strict key assertion")
            else:
